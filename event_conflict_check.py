...
def calculate_conflict_time(new_start, new_end, curr_start, curr_end) -> float:
    """
    Calculates the amount of hours in overlap between the
    new event and the existing event in the user's events list.

    :param new_start: New event's start date/time.
    :param new_end: New event's end date/time.
    :param curr_start: Current event's start date/time.
    :param curr_end: Current event's end date/time.

    :return: Conflict time between events, in hours.
    """
    
    latest_start = max(new_start, curr_start)
    earliest_end = min(new_end, curr_end)

    # Calculates conflict time in hours
    conflict_time = (earliest_end - latest_start).total_seconds() / 3600
...
def event_conflict_check(start_datetime, end_datetime, events_dict) -> list:
    ...
    for event in events_dict:
        conflict_time = calculate_conflict_time(start_datetime, end_datetime, event['start_datetime'], event['end_datetime'])
        if conflict_time > 0:
            event['conflict_time'] = conflict_time
...
def write_conflict_data(conflicts_list) -> None:
    ...
    # Sort the Event Conflicts List in Ascending Order
    conflicts_list.sort(key=lambda item: item['conflict_time'])

    # Send Event Conflicts List to Main Program
